...
const Homepage = () => {
  ...
  const search = useSelector(state=>state.allMenu.search)
  ...
  return (
    <>
      <div className='bg-green-100 pt-16'>

       {!search&&(<Banner />)}
      </ div>
      <div className="bg-green-100 min-h-screen w-full pb-16 px-4 pt-16 relative">

        <button
          onClick={handleSort}
          className='absolute left-24 top-2 border border-green-600 rounded-xl px-4 py-2 text-green-700 hover:bg-green-400 hover:text-white whitespace-nowrap'>
          {sortOrder === "null"
            ? 'Sort by Price'
            : sortOrder === "desc"
              ? 'Price: High → Low'
